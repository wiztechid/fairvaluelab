# Pipeline v3.11: robust QC with family independence and family weight caps.
...
def capped_weights(keep,cap=.60):
    ...
    # cap each economic family, not each method; redistribute excess iteratively.
    for _ in range(8):
        fams={}
...
def process(path):
    ...
    price=d.get('price');methods=d.get('methods') or [];q=d.setdefault('quality',{});guards=q.setdefault('guards',[]);keep=robust_keep(methods,price)
    ...
    d['fairValue']={'bear':bear,'base':base,'bull':bull,'dispersion':disp,'agreement':agr,'available':suf};q['validMethodCount']=len(keep) if suf else 0;q['independentFamilies']=len(ifam) if suf else 0;q['outlierMethodCount']=sum(m.get('qcStatus')=='OUTLIER' for m in methods)
    if not suf and q.get('dataLabel')=='BAIK':q['dataLabel']='CUKUP'
    fresh=d.get('freshnessStatus','FRESH');d['analysisStatus']='STALE' if fresh=='STALE' else ('SIAP' if suf else 'TERBATAS');d['engineVersion']='3.11-sector-aware-qc';json.dump(d,open(path,'w',encoding='utf-8'),ensure_ascii=False,indent=2,allow_nan=False);return d
rows={}
