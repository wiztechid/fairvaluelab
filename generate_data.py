...
TICKERS=[x.strip().upper() for x in os.getenv('TICKERS','TOTL,STAA,CMRY,PDPP,SMGA,PKPK,PORT,SMAR,GTRA,CINT,AMIN,CLPI,PSGO,UNIC,BULL,CASS,FORE,JTPE,DWGL').split(',') if x.strip()];OUT='data';os.makedirs(OUT,exist_ok=True)
def n(x):
...
def profile(sector,industry):
 x=((sector or '')+' '+(industry or '')).lower()
 # relevance multipliers: methods remain transparent; sector changes their influence.
 if any(k in x for k in ['bank','financial','insurance']):return 'Keuangan',{'DCF':.20,'FCF Yield':.15,'Historical P/E':.80,'Historical PBV':1.50},'PBV/ROE menjadi acuan utama; arus kas bebas kurang representatif untuk bisnis keuangan.'
 if any(k in x for k in ['real estate','property','construction']):return 'Properti & Real Estat',{'DCF':.75,'FCF Yield':.55,'Historical P/E':.85,'Historical PBV':1.25},'PBV mendapat bobot lebih besar karena aset dan nilai buku penting; DCF/P-E tetap dipakai sebagai pembanding.'
 if any(k in x for k in ['energy','oil','gas','coal','mining','metal','materials']):return 'Energi / Komoditas',{'DCF':.65,'FCF Yield':1.10,'Historical P/E':.80,'Historical PBV':.55},'FCF dan laba historis dinormalisasi lebih penting karena laba komoditas bersifat siklikal.'
 if any(k in x for k in ['consumer','food','beverage','retail','household']):return 'Konsumer',{'DCF':1.00,'FCF Yield':1.00,'Historical P/E':1.20,'Historical PBV':.55},'P/E, DCF dan FCF lebih relevan untuk bisnis konsumsi dengan laba dan arus kas berulang.'
 if any(k in x for k in ['technology','software','internet']):return 'Teknologi',{'DCF':1.15,'FCF Yield':.85,'Historical P/E':1.00,'Historical PBV':.25},'Pertumbuhan dan arus kas lebih penting; PBV diberi bobot rendah.'
 if any(k in x for k in ['infrastructure','telecom','transport','logistic','utility']):return 'Infrastruktur / Transportasi',{'DCF':1.20,'FCF Yield':1.00,'Historical P/E':.80,'Historical PBV':.65},'DCF dan FCF lebih penting untuk bisnis beraset besar dengan arus kas jangka panjang.'
 if any(k in x for k in ['health','pharma']):return 'Kesehatan',{'DCF':1.00,'FCF Yield':.90,'Historical P/E':1.15,'Historical PBV':.45},'P/E dan DCF menjadi acuan utama; PBV hanya pendukung.'
 return sector or 'Umum',{'DCF':.90,'FCF Yield':.90,'Historical P/E':1.00,'Historical PBV':.65},'Bobot seimbang digunakan karena profil sektor belum membutuhkan perlakuan khusus.'
def tech(h):
...
def analyze(sym):
 ...
 sector=info.get('sector') or info.get('sectorDisp');industry=info.get('industry') or info.get('industryDisp');sector_label,sw,sector_note=profile(sector,industry);price=n(h.Close.dropna().iloc[-1]);inc=t.income_stmt;bs=t.balance_sheet;cf=t.cashflow;shares=n(info.get('sharesOutstanding') or info.get('impliedSharesOutstanding'));mcap=n(info.get('marketCap'))
 ...
 def add(name,bear,base,bull,conf,relevance,bands=None,warning=None,explain=''):
  vals=[n(bear),n(base),n(bull)]
  if all(v is not None and v>0 for v in vals):methods.append({'name':name,'bear':vals[0],'base':vals[1],'bull':vals[2],'confidence':conf,'relevance':rel(name,relevance),'rawWeight':conf*rel(name,relevance),'bands':bands,'warning':warning,'explanation':explain})
