...
def series(df,names):
 ...
 for k in names:
  if k in df.index:return sorted([(pd.Timestamp(d),float(v)) for d,v in df.loc[k].items() if pd.notna(v)])
 return []
def cagr(a):
...
def tech(h):
 ...
 return {'ma20':ma(20),'ma50':ma(50),'ma200':ma(200),'rsi14':n((100-100/(1+rs)).iloc[-1]),'macd':n(m.iloc[-1]),'macdSignal':n(s.iloc[-1]),'return1m':n(last/c.iloc[-22]-1) if len(c)>=22 else None,'return3m':n(last/c.iloc[-66]-1) if len(c)>=66 else None,'low52':n(c.tail(252).min()),'high52':n(c.tail(252).max())}
